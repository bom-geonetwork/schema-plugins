		...
		<records>
			<xsl:for-each select="tds:catalog/tds:dataset[1 and $datasetName!='latest.xml']">
				...
				<record uuid="{$uuid}">
					...
					<fragment id="thredds.metadata.creation" uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_metadata_creation')}">
						<gmd:dateStamp>
							<gco:DateTime>
								<xsl:variable name="creationDate" select="tds:date[@type='created']|tds:metadata//tds:date[@type='created']"/>
								
								<xsl:choose>
									<xsl:when test="$creationDate[1]!=''">
										<!-- Use resource creation date if available -->
										<xsl:call-template name="getThreddsDateAsUTC"><xsl:with-param name="sourceDate" select="$creationDate[1]"/></xsl:call-template>
									</xsl:when>
									<xsl:otherwise>
					...
					<replacementGroup id="thredds.resource.dates">
						...
						<xsl:if test="$creationDate[1]!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_resource_creation')}">
								<gmd:date>
									<gmd:CI_Date>
										<gmd:date>
											<xsl:call-template name="getThreddsDateAsUTC"><xsl:with-param name="sourceDate" select="$creationDate[1]"/></xsl:call-template>
						...
						<xsl:variable name="modifiedDate" select="tds:date[@type='modified']|tds:metadata//tds:date[@type='modified']"/>
						
						<xsl:if test="$modifiedDate[1]!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_resource_revision')}">
								<gmd:date>
									<gmd:CI_Date>
										<gmd:date>
											<gco:DateTime>
												<xsl:call-template name="getThreddsDateAsUTC"><xsl:with-param name="sourceDate" select="$modifiedDate[1]"/></xsl:call-template>
											</gco:DateTime>
										</gmd:date>
										<gmd:dateType>
											<gmd:CI_DateTypeCode
						...
						<xsl:variable name="issuedDate" select="tds:date[@type='issued']|tds:metadata//tds:date[@type='issued']"/>
						
						<xsl:if test="$issuedDate[1]!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_resource_published')}">
								<gmd:date>
									<gmd:CI_Date>
										<gmd:date>
											<gco:DateTime>
												<xsl:call-template name="getThreddsDateAsUTC"><xsl:with-param name="sourceDate" select="$issuedDate[1]"/></xsl:call-template>
											</gco:DateTime>
										</gmd:date>
										<gmd:dateType>
											<gmd:CI_DateTypeCode
					...
					<replacementGroup id="thredds.responsible.parties">
						...
						<xsl:for-each select="tds:publisher|tds:metadata//tds:publisher">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_publisher')}">
														...
														<gmd:address>
															<gmd:CI_Address>
																<gmd:electronicMailAddress>
																	<gco:CharacterString><xsl:value-of select="tds:contact/@email"/></gco:CharacterString>
																</gmd:electronicMailAddress>
															</gmd:CI_Address>
														</gmd:address>
													</xsl:if>
													<xsl:if test="tds:contact/@url!=''">
														<gmd:onlineResource>
															<gmd:CI_OnlineResource>
																<gmd:linkage>
																	<gmd:URL><xsl:value-of select="tds:contact/@url"/></gmd:URL>
																</gmd:linkage>
																<gmd:protocol>
						...
						<xsl:for-each select="tds:creator|tds:metadata//tds:creator">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_originator')}">
								<gmd:citedResponsibleParty>
									<gmd:CI_ResponsibleParty>
										<gmd:individualName>
											<gco:CharacterString>
						...
						<xsl:for-each select="tds:contributor|tds:metadata//tds:contributor">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_author')}">
								<gmd:citedResponsibleParty>
									<gmd:CI_ResponsibleParty>
										<gmd:individualName>
											<gco:CharacterString>
					...
					<fragment id="thredds.abstract" uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_abstract')}">
						<gmd:abstract>
							<gco:CharacterString>
								<xsl:choose>
									<xsl:when test="tds:documentation[@type='summary']|tds:metadata//tds:documentation[@type='summary']">
										<xsl:for-each select="tds:documentation[@type='summary']|tds:metadata//tds:documentation[@type='summary']">
											<xsl:value-of select="."/>
										</xsl:for-each>
									</xsl:when>
					...
					<replacementGroup id="thredds.credit">
						<xsl:for-each select="tds:documentation[@type='funding']|tds:metadata//tds:documentation[@type='funding']">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_credit')}">
								<gmd:credit>
					...
					<replacementGroup id="thredds.keywords">
						...
						<xsl:if test="tds:keyword[not(@vocabulary)]|tds:metadata//tds:keyword[not(@vocabulary)]">
							<xsl:call-template name="descriptive-keywords">
								<xsl:with-param name="keywords" select="tds:keyword[not(@vocabulary)]|tds:metadata//tds:keyword[not(@vocabulary)]"/>
							</xsl:call-template>
						</xsl:if>
		
						<!-- Add keywords belonging to a vocabulary -->
						<xsl:for-each-group select="tds:keyword|tds:metadata//tds:keyword" group-by="@vocabulary">
							<xsl:call-template name="descriptive-keywords">
					...
					<replacementGroup id="thredds.use.constraints">
						<xsl:for-each select="tds:documentation[@type='rights']|tds:metadata//tds:documentation[@type='rights']">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_use_constraints')}">
					...
					<replacementGroup id="thredds.project">
						<xsl:for-each select="tds:project|tds:metadata//tds:project">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_project')}">
								<gmd:aggregationInfo>
					...
					<fragment id="thredds.extent" uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_extent')}">
						<gmd:extent>
							<gmd:EX_Extent>
								...
								<xsl:for-each select="tds:geospatialCoverage|tds:metadata//tds:geospatialCoverage">
									...
									<xsl:if	test="tds:northsouth/tds:units='degrees_north' and tds:eastwest/tds:units='degrees_east'">
										<gmd:geographicElement>
											<gmd:EX_GeographicBoundingBox>
												...
												<gmd:northBoundLatitude>
													<gco:Decimal>
														<xsl:value-of	select="tds:northsouth/tds:start + tds:northsouth/tds:size"/>
													</gco:Decimal>
												</gmd:northBoundLatitude>
											</gmd:EX_GeographicBoundingBox>
										</gmd:geographicElement>
								...
								<xsl:for-each select="tds:timeCoverage|tds:metadata//tds:timeCoverage">
									...
									<gmd:temporalElement>
										<gmd:EX_TemporalExtent>
											<gmd:extent>
												<gml32:TimePeriod gml32:id="{generate-id(.)}">
													<gml32:beginPosition>
								...
								<xsl:for-each select="tds:geospatialCoverage/tds:updown|tds:metadata//tds:geospatialCoverage/tds:updown">
									<gmd:verticalElement>
										<gmd:EX_VerticalExtent>
											<gmd:minimumValue>
												<gco:Real><xsl:value-of select="tds:start"/></gco:Real>
											</gmd:minimumValue>
											<gmd:maximumValue>
												<gco:Real><xsl:value-of select="tds:start+tds:size"/></gco:Real>
											</gmd:maximumValue>
										</gmd:EX_VerticalExtent>
									</gmd:verticalElement>
								</xsl:for-each>
					...
					<replacementGroup id="thredds.supplemental">
						<xsl:if test="tds:documentation[not(@type)]|tds:metadata//tds:documentation[not(@type)]">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_supplemental')}">
								<gmd:supplementalInformation>
									<gco:CharacterString>
										<xsl:for-each select="tds:documentation[not(@type)]|tds:metadata//tds:documentation[not(@type)]">
											<xsl:value-of select="."/>
										</xsl:for-each>
									</gco:CharacterString>
								</gmd:supplementalInformation>
					...
					<replacementGroup id="thredds.online.resources">
						...
						<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_dataset_link')}">
							<gmd:onLine>
								<gmd:CI_OnlineResource>
									<gmd:linkage>
										<gmd:URL>
						...
						<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_catalog_link')}">
							<gmd:onLine>
								<gmd:CI_OnlineResource>
									<gmd:linkage>
										<gmd:URL>
					...
					<replacementGroup id="thredds.data.quality">
						<xsl:for-each select="tds:documentation[@type='history']|tds:metadata//tds:documentation[@type='history']">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_history')}">
								<gmd:dataQualityInfo>
									<gmd:DQ_DataQuality>
										<gmd:scope>
											<gmd:DQ_Scope>
						...
						<xsl:for-each select="tds:documentation[@type='processing_level']|tds:metadata//tds:documentation[@type='processing_level']">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_processing_level')}">
								<gmd:dataQualityInfo>
									<gmd:DQ_DataQuality>
										<gmd:scope>
											<gmd:DQ_Scope>
	...
	<xsl:template match="tds:service" mode="transfer-options">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($datasetName,'_service_url')}">
					<gmd:onLine>
						<gmd:CI_OnlineResource>
							<gmd:linkage>
