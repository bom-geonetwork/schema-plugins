...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- 
    Load the schema configuration for the editor.
      -->
  <xsl:template name="get-iso19115-2-configuration">
    <xsl:copy-of select="document('config-editor.xml')"/>
  </xsl:template>


  <xsl:template name="dispatch-iso19115-2">
  ...
  <!-- Evaluate an expression. This is schema dependant in order to properly 
    ...
    -->
  <xsl:template name="evaluate-iso19115-2">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
  ...
  <xsl:template name="evaluate-iso19115-2-boolean">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
