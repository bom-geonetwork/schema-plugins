...
<xsl:template match="gml32:timePosition[not(@gco:nilReason)]">
    <xsl:choose>
...
<xsl:template match="gml32:beginPosition[not(@gco:nilReason)]">
    <xsl:choose>
        <xsl:when test='contains(., "T")'>
            <xsl:value-of select="."/>
        </xsl:when>
...
<xsl:template match="gml32:endPosition[not(@gco:nilReason)]">
    <xsl:choose>
        <xsl:when test='contains(., "T")'>
            <xsl:value-of select="."/>
        </xsl:when>
...
<xsl:template match="gml32:coordinates">
	<xsl:element name="spatial">
		<xsl:attribute name="type">
...
-->
<xsl:template match="gmi:MI_Metadata" mode="collection">
...
<!-- the originating source --> 
  <xsl:param name="origSource" select="/root/env/siteURL"/>
	...
	<xsl:variable name="formattedFrom">
		<xsl:choose>
			<xsl:when test="$te[1]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:beginPosition">
				<xsl:value-of select="$te[1]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:beginPosition"/>
			</xsl:when>
			<xsl:when test="$te[1]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:begin/gml32:TimeInstant/gml32:timePosition">
				<xsl:value-of select="$te[1]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:begin/gml32:TimeInstant/gml32:timePosition"/>
			</xsl:when>
		</xsl:choose>					
	</xsl:variable>
	...
	<xsl:variable name="formattedTo">
		<xsl:choose>
			<xsl:when test="$te[position()=last()]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:endPosition">
				<xsl:value-of	select="$te[position()=last()]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:endPosition"/>
			</xsl:when>
			<xsl:when test="$te[position()=last()]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:end/gml32:TimeInstant/gml32:timePosition">
				<xsl:value-of select="$te[position()=last()]/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod/gml32:end/gml32:TimeInstant/gml32:timePosition"/>
			</xsl:when>
		</xsl:choose>
	</xsl:variable>
		...
		<xsl:element name="collection">
			...
			<xsl:if test="$ge/gmd:EX_GeographicBoundingBox">
				<xsl:element name="location">
					...
					<xsl:apply-templates select="$ge/gmd:EX_BoundingPolygon/gmd:polygon/gml32:Polygon/gml32:exterior/gml32:LinearRing/gml32:coordinates[text()!='']"/>
           ...
           <xsl:for-each-group select="descendant::gmd:CI_ResponsibleParty[(gmd:individualName[not(@gco:nilReason)] or gmd:positionName[not(@gco:nilReason)] or gmd:organisationName[not(@gco:nilReason)]) and not(gmd:role/gmd:CI_RoleCode/@codeListValue='')]"  group-by="gmd:role/gmd:CI_RoleCode">
			...
			<xsl:for-each-group select="descendant::gmd:CI_ResponsibleParty[gmd:organisationName[not(@gco:nilReason)] and not(gmd:role/gmd:CI_RoleCode/@codeListValue='') and gmd:individualName='']" group-by="gmd:organisationName">
				<xsl:element name="relatedObject">
					<xsl:element name="key">
						<xsl:value-of select="current-grouping-key()"/>
					</xsl:element>
			...
			<xsl:if test="$te/gmd:EX_TemporalExtent/gmd:extent/gml32:TimePeriod">
   ...
   <xsl:for-each-group select="descendant::gmd:CI_ResponsibleParty[(gmd:individualName[not(@gco:nilReason)] or gmd:positionName[not(@gco:nilReason)] or gmd:organisationName[not(@gco:nilReason)]) and not(gmd:role/gmd:CI_RoleCode/@codeListValue='')]" group-by="gmd:role/gmd:CI_RoleCode">
		<xsl:element name="registryObject">
			...
			<xsl:element name="party">
				...
				<xsl:element name="relatedObject">
					<xsl:element name="key">
						<xsl:value-of select="ancestor::gmi:MI_Metadata/gmd:fileIdentifier"/>
					</xsl:element>	
	...
	<xsl:for-each-group select="descendant::gmd:CI_ResponsibleParty[gmd:individualName[@gco:nilReason] and gmd:organisationName[not(@gco:nilReason)] and not(gmd:role/gmd:CI_RoleCode/@codeListValue='')]" group-by="gmd:organisationName">
		<xsl:element name="registryObject">
			<xsl:attribute name="group">
				<xsl:value-of select="$group"/>
			</xsl:attribute>
