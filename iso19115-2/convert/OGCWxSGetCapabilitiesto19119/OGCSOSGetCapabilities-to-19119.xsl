...
<xsl:stylesheet version="2.0" 
	...
	xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gmd="http://www.isotc211.org/2005/gmd"
	xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gml32="http://www.opengis.net/gml/3.2"
	xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sos="http://www.opengis.net/sos/1.0"
	xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:ows="http://www.opengis.net/ows/1.1"
	xmlns:crs="java:org.geotools.referencing.CRS" xmlns:defcrs="java:org.geotools.referencing.crs.DefaultGeographicCRS"
	xmlns:renv="java:org.geotools.geometry.jts.ReferencedEnvelope" xmlns:xs="http://www.w3.org/2001/XMLSchema" 
		...
		<gmi:MI_Metadata>
							...
							<RS_Identifier>
								<code>
									...
									<xsl:choose>
										...
										<xsl:otherwise>
											<gco:CharacterString><xsl:value-of select="//sos:ObservationOffering[gml:srsName][1]/gml:srsName[1]"/></gco:CharacterString>
										</xsl:otherwise>
									</xsl:choose>
								</code>
							</RS_Identifier>
			...
			<identificationInfo>
				<srv:SV_ServiceIdentification>
					...
					<status>
						<xsl:choose>
							<xsl:when test="$temporalExtent/gmd:temporalElement/gmd:EX_TemporalExtent/gmd:extent/gml:TimePeriod/gml:endPosition/@indeterminatePosition[.='now']">
								<MD_ProgressCode codeList="./resources/codeList.xml#MD_ProgressCode" codeListValue="onGoing" />
							</xsl:when>
							<xsl:otherwise>
						...
						<MD_DigitalTransferOptions>
							<onLine>
								<CI_OnlineResource>
									<linkage>
										<URL>
											<xsl:value-of select="ows:OperationsMetadata/ows:Operation[@name='GetCapabilities']/ows:DCP/ows:HTTP/ows:Get/@xlink:href"/>
										</URL>
									</linkage>
									<protocol>
										<gco:CharacterString>WWW:LINK-1.0-http--link</gco:CharacterString>
			...
			<bbox xmlns="">
				<xsl:choose>
					...
					<xsl:when test="@srsName='urn:ogc:def:crs:epsg::4326'">
						<southBL><xsl:value-of select="$xmin"/></southBL>
						<westBL><xsl:value-of select="$ymin"/></westBL>
						<northBL><xsl:value-of select="$xmax"/></northBL>
						<eastBL><xsl:value-of select="$ymax"/></eastBL>
					</xsl:when>
					<!-- Reproject envelope to WGS84 if relevant geotools functions are available -->
					<xsl:when test="function-available('crs:decode') and function-available('renv:new') and function-available('renv:transform')">
						...
						<southBL><xsl:value-of select="renv:getMinY($envWGS84)"/></southBL>
						<westBL><xsl:value-of select="renv:getMinX($envWGS84)"/></westBL>
						<northBL><xsl:value-of select="renv:getMaxY($envWGS84)"/></northBL>
						<eastBL><xsl:value-of select="renv:getMaxX($envWGS84)"/></eastBL>
					</xsl:when>
	...
	<xsl:template name="bounding-box">
		...
		<EX_GeographicBoundingBox>
			<westBoundLongitude>
				<gco:Decimal><xsl:copy-of select="min($bboxes/bbox/westBL)"/></gco:Decimal>
			</westBoundLongitude>
			<eastBoundLongitude>
				<gco:Decimal><xsl:value-of select="max($bboxes/bbox/eastBL)"/></gco:Decimal>
			</eastBoundLongitude>
			<southBoundLatitude>
				<gco:Decimal><xsl:value-of select="min($bboxes/bbox/southBL)"/></gco:Decimal>
			</southBoundLatitude>
			<northBoundLatitude>
				<gco:Decimal><xsl:value-of select="max($bboxes/bbox/northBL)"/></gco:Decimal>
			</northBoundLatitude> 
		</EX_GeographicBoundingBox>
	</xsl:template>
	...
	<xsl:template name="bounding-polygon">
		<EX_BoundingPolygon>
			<xsl:for-each select="//sos:ObservationOffering/gml:boundedBy/gml:Envelope">
				...
				<polygon>
					<gml32:Polygon gml:id="{generate-id()}" srsName="{@srsName}">
						<gml32:exterior>
							<gml32:LinearRing>
								<gml32:coordinates><xsl:value-of select="$ymin"/>,<xsl:value-of select="$xmax"/>, <xsl:value-of select="$ymax"/>,<xsl:value-of select="$xmax"/>, <xsl:value-of select="$ymax"/>,<xsl:value-of select="$xmin"/>, <xsl:value-of select="$ymin"/>,<xsl:value-of select="$xmin"/>, <xsl:value-of select="$ymin"/>,<xsl:value-of select="$xmax"/></gml32:coordinates>
							</gml32:LinearRing>
						</gml32:exterior>
					</gml32:Polygon>
				</polygon>
	...
	<xsl:template name="temporal-element">
		<temporalElement>
			<EX_TemporalExtent>
				<extent>
					<gml32:TimePeriod gml:id="{concat(generate-id(),'t')}">
						<gml32:beginPosition>
							<xsl:choose>
						...
						<gml32:endPosition>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select="max((//sos:ObservationOffering/sos:time/gml:TimePeriod/xs:dateTime(gml:beginPosition),//sos:ObservationOffering/sos:time/xs:dateTime(gml:TimePosition)))"/>
								</xsl:otherwise>
							</xsl:choose>
						</gml32:endPosition>
					</gml32:TimePeriod>
