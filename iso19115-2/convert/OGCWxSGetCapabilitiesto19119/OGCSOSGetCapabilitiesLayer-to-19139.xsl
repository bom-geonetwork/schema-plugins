...
<xsl:stylesheet version="2.0"
	...
	xmlns:renv="java:org.geotools.geometry.jts.ReferencedEnvelope" extension-element-prefixes="crs defcrs renv"
				...
				<MD_Distribution>
					...
					<transferOptions>
						<MD_DigitalTransferOptions>
							<xsl:if test="//ows:Operation[@name='GetObservation']/ows:DCP/ows:HTTP/ows:Post">
								<onLine>
									<CI_OnlineResource>
										<linkage>
											<URL>
												<xsl:value-of select="//ows:Operation[@name='GetObservation']/ows:DCP/ows:HTTP/ows:Post/@xlink:href" />
											</URL>
										</linkage>
										<protocol>
							...
							<xsl:apply-templates mode="onlineResource" select="//sos:ObservationOffering[gml:name=$Name]/sos:responseFormat[//ows:Operation[@name='GetObservation']/ows:DCP/ows:HTTP/ows:Get/@xlink:href]"/>
						</MD_DigitalTransferOptions>
					</transferOptions>
				</MD_Distribution>
			</distributionInfo>
	...
	<xsl:template match="*" mode="OfferingDataIdentification">
						...
						<MD_Identifier>
							<code>
								<xsl:choose>
									...
									<xsl:otherwise>
										<gmx:Anchor xlink:href="{$codeSpace}"><xsl:value-of select="."/></gmx:Anchor>
									</xsl:otherwise>
								</xsl:choose>
							</code>
						</MD_Identifier>
		...
		<xsl:if test="//sos:ObservationOffering[gml:name=$Name]/sos:intendedApplication!=''">
			<purpose>
				<gco:CharacterString>
					<xsl:for-each select="//sos:ObservationOffering[gml:name=$Name]/sos:intendedApplication[normalize-space()!='']">
						<xsl:value-of select="."/>
		...
		<status>
			<xsl:choose>
				<xsl:when test="//sos:ObservationOffering[gml:name=$Name]/sos:time/(gml:TimePeriod/gml:endPosition|gml:TimeInstant)/@indeterminatePosition[.='now']">
			...
			<descriptiveKeywords>
				<MD_Keywords>
					<xsl:for-each select="//sos:ObservationOffering[gml:name=$Name]/sos:observedProperty">
						<keyword>
							<gco:CharacterString><xsl:value-of select="@xlink:href|gml:name"/></gco:CharacterString>
						</keyword>
					</xsl:for-each>
					<type>
		...
		<xsl:if test="//sos:ObservationOffering[gml:name=$Name]/sos:featureOfInterest">
			<descriptiveKeywords>
				<MD_Keywords>
					...
					<type>
						<MD_KeywordTypeCode codeList="http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_KeywordTypeCode" codeListValue="place">place</MD_KeywordTypeCode>
					</type>
				</MD_Keywords>
			</descriptiveKeywords>
		</xsl:if>
		...
		<extent>
			<EX_Extent>
				<xsl:apply-templates mode="spatial-extent" select="//sos:ObservationOffering[gml:name=$Name]/gml:boundedBy/gml:Envelope"/>
				<xsl:apply-templates mode="temporal-extent" select="//sos:ObservationOffering[gml:name=$Name]/sos:time"/>
			</EX_Extent>
		</extent>
	...
	<xsl:template mode="onlineResource" match="*">
		...
		<xsl:variable name="url">
			...
			<xsl:for-each select="//sos:ObservationOffering[gml:name=$Name]/sos:observedProperty/@xlink:href">
				<xsl:value-of select="."/>
				<xsl:if test="position()!=last()">,</xsl:if>
			</xsl:for-each>
